...
function buildStaticPortfolioAnswer({
  ...
  if (
    ...
    return [
      '### Моя философия на стыке Project Management и Fullstack QA:',
      '',
      '> *«Качество — это не просто отсутствие багов в трекере, а обоснованная уверенность команды и бизнеса в том, что система надежно выдержит реальные пользовательские сценарии, а продукт решает настоящую задачу бизнеса.»*',
      '',
      '**Ключевые принципы:**',
      '- **Продуктовый фокус и декомпозиция (PM):** Четко понимать цель спринта, отсекать лишнее и управлять ожиданиями стейкхолдеров, обеспечивая быстрый Time-to-Market без потери устойчивости.',
      '- **Shift-Left (QA):** Предотвращать дефекты еще до написания кода — на этапе анализа требований в Jira и макетов в Figma. Это в разы дешевле, чем чинить баги на проде.',
