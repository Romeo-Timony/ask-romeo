...
function buildStaticPortfolioAnswer({ query, retrievedContext, reason }: { query: string; retrievedContext: string; reason: 'model_unavailable' | 'rate_limit' }) {
  ...
  if (matches(normalizedQuery, ['проект', 'project', 'portfolio', 'портфолио'])) {
    return [intro, '', 'Ключевые проекты:', '', ...oosuProjects.slice(0, 5).map((project) => `- ${project.title}: ${project.description}${project.links[0]?.url ? `\n  Ссылка: ${project.links[0].url}` : ''}`)].join('\n');
  }
  if (matches(normalizedQuery, ['контакт', 'связь', 'contact', 'collab', 'github'])) {
    return [intro, '', `- GitHub: ${oosuProfile.github}`, `- LinkedIn: ${oosuProfile.linkedin}`, `- Instagram: ${oosuProfile.instagram}`, `- Email: ${oosuProfile.email}`].join('\n');
  }
  ...
  return [intro, '', `Ask Romeo — диалоговое портфолио ${oosuProfile.name}. Здесь можно спросить о проектах, QA-навыках, технологиях и сотрудничестве.`, '', `GitHub: ${oosuProfile.github}`].join('\n');
}
